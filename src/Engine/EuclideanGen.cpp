...
uint32_t GenerateEuclidean(int hits, int steps)
{
    ...
    // Special cases
    if (hits == 0) return 0;
    if (hits >= steps) return (1u << steps) - 1;  // All steps
...
{
    if (steps < 1 || steps > 32) return pattern;
    ...
    uint32_t mask = (1u << steps) - 1;

    // Rotate right by offset
    uint32_t rotated = ((pattern >> offset) | (pattern << (steps - offset))) & mask;
    ...
    uint32_t seed)
{
    ...
    {
        uint32_t euclidean = GenerateEuclidean(budget, steps);
        // Rotate by seed-derived offset
        int rotation = static_cast<int>(seed % static_cast<uint32_t>(steps));
        uint32_t rotated = RotatePattern(euclidean, rotation, steps);
    ...
    uint32_t euclideanPattern = GenerateEuclidean(euclideanHits, steps);
    int rotation = static_cast<int>(seed % static_cast<uint32_t>(steps));
    ...
    uint32_t remainingEligibility = eligibility & ~euclideanPattern;
    uint32_t gumbelPattern = SelectHitsGumbelSimple(weights, remainingEligibility, gumbelHits, seed ^ 0xE0C1, steps);
